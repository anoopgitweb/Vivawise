...
type AdminAssignment = { id:string; user_id:string; assigned_at:string; user?:AdminUser };
type AdminTopic = AssignedTopic & { document_count?: number; assignment_count?: number; assignments?:AdminAssignment[] };
function AdminPanel() {
  const [authenticated, setAuthenticated] = useState(true); const [topics, setTopics] = useState<AdminTopic[]>([]); const [users,setUsers]=useState<AdminUser[]>([]); const [message, setMessage] = useState("");
  ...
  const [selected, setSelected] = useState(""); const [selectedUser,setSelectedUser]=useState("");
  const load = () => fetch("/api/admin/topics").then(async (r) => { if (!r.ok) throw new Error(); return r.json(); }).then((d) => { setAuthenticated(true); setTopics(d.topics ?? []);setUsers(d.users??[]); setSelected((v) => v || d.topics?.[0]?.id || ""); }).catch(() => setAuthenticated(false));
  useEffect(load, []);
  ...
  async function assign(event: React.FormEvent) { event.preventDefault(); const r = await fetch("/api/admin/topics", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "assign", topicId: selected, userId:selectedUser }) }); const d = await r.json(); if (!r.ok) return setMessage(d.error); setSelectedUser(""); setMessage("Student assigned successfully."); load(); }
  ...
  return <div className="page"><div className="page-heading"><div><span className="eyebrow">ADMIN CONTROL CENTRE</span><h1>Mock viva assignments</h1><p>Create named tests, attach source material and assign registered students.</p></div></div>{message && <div className="admin-message">{message}</div>}<div className="admin-grid"><form className="admin-card" onSubmit={create}><h2>Create mock viva</h2><label>Title<input value={title} required onChange={(e) => setTitle(e.target.value)} placeholder="DBMS Semester Viva" /></label><label>Subject<input value={subject} required onChange={(e) => setSubject(e.target.value)} placeholder="Database Systems" /></label><label>Description<textarea value={description} onChange={(e) => setDescription(e.target.value)} /></label><label>Difficulty<select value={difficulty} onChange={(e) => setDifficulty(e.target.value as Difficulty)}><option>Foundation</option><option>Standard</option><option>Challenge</option></select></label><button className="primary-button">Create topic</button></form><section className="admin-card"><h2>Assign & upload</h2><label>Mock viva<select value={selected} onChange={(e) => setSelected(e.target.value)}><option value="">Choose test</option>{topics.map((t) => <option value={t.id} key={t.id}>{t.title}</option>)}</select></label><form onSubmit={assign}><label>Registered student<select value={selectedUser} required onChange={e=>setSelectedUser(e.target.value)}><option value="">Choose student</option>{users.map(user=><option key={user.id} value={user.id}>{user.full_name||user.email} — {user.email}</option>)}</select></label>{!users.length&&<small>No student accounts have registered yet.</small>}<button className="primary-button" disabled={!selected||!selectedUser}>Assign student</button></form><label className="admin-upload">Supporting document<input type="file" accept=".pdf,.doc,.docx,.txt" onChange={upload} disabled={!selected} /></label></section></div><section className="admin-topic-list"><h2>Existing assignments</h2>{topics.length ? topics.map((t) => <div className="assignment-group" key={t.id}><button onClick={() => setSelected(t.id)}><div><strong>{t.title}</strong><span>{t.subject} · {t.difficulty}</span></div><span>{t.document_count ?? 0} docs · {t.assignment_count ?? 0} students</span></button><div className="assigned-users">{t.assignments?.length?t.assignments.map(a=><span key={a.id}><b>{a.user?.full_name||"Student"}</b>{a.user?.email}</span>):<em>No students assigned</em>}</div></div>) : <p>No mock vivas created yet.</p>}</section></div>;
}
